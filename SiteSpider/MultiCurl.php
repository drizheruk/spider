...
class MultiCurl {
    ...
    function multiget($urls) {
		...
		curl_setopt($conn[$i], CURLOPT_FOLLOWLOCATION, TRUE);
            ...
            foreach ($pack as $i => $url) {
                $result[$i]['content'] = curl_multi_getcontent($conn[$i]);
                $info = curl_getinfo($conn[$i]);
                $result[$i]['http_code'] = $info['http_code'];
                $result[$i]['page_size'] = $info['size_download'];
                $result[$i]['content_type'] = $info['content_type'];
                $result[$i]['redirect_url'] = $info['redirect_url'];
            }
            curl_multi_close($mh);
        }
        return $result;
